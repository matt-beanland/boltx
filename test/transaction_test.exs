defmodule Transaction.Test do
  ...
  describe "Transactions" do
    ...
    test "execute statements in transaction", c do
      Bolty.transaction(c.conn, fn conn ->
        book =
          Bolty.query!(
            conn,
        ...
        Bolty.rollback(conn, :changed_my_mind)
      end)
    ...
    test "rollback statements in transaction", c do
      try do
        ...
        {:ok, %Response{results: [result]}} =
          Bolty.query(c.conn, "MATCH (x:XactRollback) RETURN count(x)")
      ...
      after
        ...
        Bolty.query(c.conn, "MATCH (x:XactRollback) DETACH DELETE x")
      end
    end
    ...
    test "commit statements in transaction", c do
      try do
        Bolty.transaction(c.conn, fn conn ->
          assert {:ok, %Response{results: books}} =
                   Bolty.query(conn, "CREATE (x:XactCommit {foo: 'bar'}) return x")
          ...
          {:ok, %Response{results: results}} =
            Bolty.query(c.conn, "MATCH (x:XactCommit) RETURN x")
      ...
      after
        ...
        Bolty.query(c.conn, "MATCH (x:XactCommit) DETACH DELETE x")
      end
    end
  end
